...
This is a checked-in copy, not a submodule, so working against the corpus never requires network access. To refresh, sparse-clone upstream, re-copy the two directories, and update the commit SHA above.

Upstream is licensed AGPL-3.0; these files are test fixtures only and are not compiled into or distributed with squill.
