//! The safety oracle's primitives: token equivalence and comment conservation
//! between input and formatted output.
//!
...
fn resolve_ident(token: &str, dialect: Dialect) -> String {
	let (inner, quoted) = if let Some(body) = token.strip_prefix('"') {
		(body.strip_suffix('"').unwrap_or(body).replace("\"\"", "\""), true)
...
pub fn comments_conserved(
	...
	comment_texts(input, dialect, lex_options)
		== comment_texts(output, dialect, lex_options)
}
