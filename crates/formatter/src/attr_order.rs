				...
				if skip_words > 0 {
					...
					if let Some(key) = sort_key(word, next) {
						...
						skip_words = match word.as_str() {
							...
							"returns" => 1,
							"language" | "security" | "external" | "parallel" | "support" => {
								1
							}
							_ => 0,
						};
...
mod tests {
	...
	fn order(sql: &str) -> Option<String> {
		let tokens: Vec<_> =
			lex_with(sql, Dialect::Postgres, LexOptions::default())
				.into_iter()
				.filter(|t| !t.kind.is_trivia())
				.collect();
		let words: Vec<W> =
			tokens.iter().map(|t| classify(t.kind, t.text)).collect();
	...
	fn type_colliding_with_keyword_is_not_a_segment() {
		...
		assert_eq!(
			order(
				"create function f() as $$x$$ returns setof language language sql;"
			),
			Some(
				"create function f ( ) returns setof language language sql as $$x$$ ;"
					.into()
			)
		);
	}
