...
//!
//! Iterative Prettier-style printer. `fits` measures a candidate flat layout
//! *plus the rest of the current line* (the pending command stack), so trailing
//! text like a `,` after a group is accounted for.
...
pub(crate) fn render(doc: &Doc, options: &Options) -> String {
	let mut printer =
		Printer { options, out: String::new(), col: 0, at_line_start: true };
	let mut stack =
		vec![Cmd { indent: 0, mode: Mode::Break, work: Work::Doc(doc) }];
...
impl Printer<'_> {
	fn step<'a>(&mut self, cmd: Cmd<'a>, stack: &mut Vec<Cmd<'a>>) {
		...
		let doc = match work {
			...
			Work::FillContent(doc) => {
				let content_mode = if self
					.fits(Cmd { indent, mode: Mode::Flat, work: Work::Doc(doc) }, stack)
				{
					Mode::Flat
				} else {
					Mode::Break
				};
				stack.push(Cmd { indent, mode: content_mode, work: Work::Doc(doc) });
				return;
			}
		};
		match doc {
			...
			Doc::Group(inner) => {
				let inner_mode = match mode {
					...
					Mode::Break => {
						if self.fits(
							Cmd { indent, mode: Mode::Flat, work: Work::Doc(inner) },
							stack,
						) {
							Mode::Flat
						} else {
				...
				stack.push(Cmd { indent, mode: inner_mode, work: Work::Doc(inner) });
			}
			Doc::Indent(inner) => {
				stack.push(Cmd { indent: indent + 1, mode, work: Work::Doc(inner) })
			}
			Doc::SoftLine => {
				if mode == Mode::Break {
			...
			Doc::IfBreak { broken, flat } => {
				...
				stack.push(Cmd { indent, mode, work: Work::Doc(chosen) });
			}
			Doc::Fill(items) => match items.split_first() {
				None => {}
				Some((first, rest)) => {
					stack.push(Cmd { indent, mode, work: Work::FillRest(rest) });
					stack.push(Cmd { indent, mode, work: Work::FillContent(first) });
				}
			},
		}
	}
	...
	fn fill_rest<'a>(
		...
		let Some((content, rest)) = rest.split_first() else {
			...
			stack.push(Cmd { indent, mode: Mode::Break, work: Work::Doc(separator) });
			return;
		};
		stack.push(Cmd { indent, mode: Mode::Break, work: Work::FillRest(rest) });
		let pair_fits = self.fits_pair(separator, content, indent, stack);
		if pair_fits {
			stack.push(Cmd { indent, mode: Mode::Flat, work: Work::Doc(content) });
			stack.push(Cmd { indent, mode: Mode::Flat, work: Work::Doc(separator) });
		} else {
			stack.push(Cmd {
				indent,
				mode: Mode::Break,
	...
	fn fits_pair(
		...
		self.fits_many(
			&[
				Cmd { indent, mode: Mode::Flat, work: Work::Doc(separator) },
				Cmd { indent, mode: Mode::Flat, work: Work::Doc(content) },
			],
			stack,
		)
	}
		...
		loop {
			...
			match doc {
				...
				Doc::Group(inner) | Doc::Indent(inner) => {
					queue.push(Cmd { indent, mode, work: Work::Doc(inner) });
				}
				Doc::SoftLine => {
					if mode == Mode::Break {
						return true;
				...
				Doc::IfBreak { broken, flat } => {
					...
					queue.push(Cmd { indent, mode, work: Work::Doc(chosen) });
				}
				Doc::Fill(items) => {
					queue.extend(items.iter().rev().map(|doc| Cmd {
						indent,
