...
//! render time.
//!
//! Postgres folds unquoted identifiers to lowercase (ASCII letters only),
//! so quotes may be stripped only from all-lowercase, identifier-shaped,
...
//! unquoted; backtick/bracket quoting is normalized to `"` when quoting
//! is kept. No regex anywhere: shapes are checked char-by-char.

use parser::Dialect;

use crate::doc::IdentPos;
use crate::keywords::{PgKeywordCategory, is_sqlite_keyword, pg_keyword_category};
use crate::{IdentQuoting, Options};
...
pub(crate) fn render_ident(text: &str, pos: IdentPos, options: &Options) -> String {
    ...
    match dialect {
        Dialect::Postgres => {
            ...
            match pg_keyword_category(name) {
                ...
                Some(PgKeywordCategory::TypeFuncName) => pos == IdentPos::TypeOrFunction,
