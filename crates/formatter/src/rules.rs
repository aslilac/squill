...
use crate::doc::concat;
use crate::doc::fill;
use crate::doc::fresh_line;
use crate::doc::group;
use crate::doc::hard_line;
use crate::doc::ident;
...
fn element_has_comment(element: SyntaxElement) -> bool {
	match element {
		...
		SyntaxElement::Node(node) => {
			node.children_with_tokens().any(element_has_comment)
		}
	}
}
...
pub(crate) fn lower_statement(
	...
	match stmt.kind() {
		...
		| SyntaxKind::PlNull => {
			...
			for token in
				stmt.descendants_with_tokens().filter_map(|el| el.into_token())
			{
...
impl Lowerer {
	...
	fn buffer_leading(&mut self, doc: Doc) {
		self.pending.push(PendingComment {
			doc,
			blank_before: self.pending_blank
				&& (self.emitted_any || !self.pending.is_empty()),
		});
	...
	fn node(&mut self, node: &SyntaxNode) -> Doc {
		match node.kind() {
			...
			SyntaxKind::ErrorStatement => {
				verbatim(node.to_string().trim().to_string())
			}
			...
			SyntaxKind::ParenSelect | SyntaxKind::SubqueryExpr => {
			...
			SyntaxKind::QuantifiedExpr | SyntaxKind::InExpr => {
				self.tail_paren_expr(node)
			}
			...
			SyntaxKind::FilterClause
			| SyntaxKind::WithinGroupClause
			| SyntaxKind::OverClause => self.space_flow(node, IdentPos::ColumnOrTable),
	...
	fn select_core(&mut self, node: &SyntaxNode) -> Doc {
		...
		for element in node.children_with_tokens() {
			match element {
				...
				SyntaxElement::Node(child)
					if child.kind() == SyntaxKind::SelectList =>
				{
					seen_list = true;
	...
	fn ddl_statement(&mut self, docs: &mut Vec<Doc>, node: &SyntaxNode) {
		...
		if words.first().is_some_and(|w| w == "create") {
			match object {
				Some("trigger") => {
					break_before =
						&["before", "after", "instead", "for", "when", "execute"];
		...
		for &element in elements {
			match element {
				...
				SyntaxElement::Token(token) => {
					...
					// inside it.
					let in_no_phrase = after_no
						&& break_before.iter().any(|kw| kw.eq_ignore_ascii_case("no"));
	...
	fn kw_clause(&mut self, node: &SyntaxNode) -> Doc {
		...
		for element in node.children_with_tokens() {
			match element {
				...
				SyntaxElement::Token(token)
					if in_head && token.kind() == SyntaxKind::Ident =>
				{
					if !head_first {
						head.push(space());
					}
	...
	fn when_clause(&mut self, node: &SyntaxNode) -> Doc {
		...
		group(concat([
			concat(head),
			indent(concat(
				[soft_line_or_space()].into_iter().chain(then).collect::<Vec<_>>(),
			)),
		]))
	}
				...
				SyntaxElement::Node(child) => {
					let doc = match child.kind() {
						SyntaxKind::ColumnRef => {
							self.column_ref(child, IdentPos::TypeOrFunction)
						}
						_ => self.node(child),
					};
	...
	/// A column definition or table constraint inside a CREATE TABLE
	/// list: space-joined like [`Self::space_flow`], but with fill break
	/// points before constraint-starting keywords. An overlong item
	/// wraps before `references` / `default` / `unique` / ... (each
	...
	fn column_def(&mut self, node: &SyntaxNode) -> Doc {
		...
		let words: Vec<(usize, String)> = elements
			...
			.filter_map(|(at, element)| match element {
				SyntaxElement::Token(token)
					if token.kind() == SyntaxKind::Ident && !token.kind().is_trivia() =>
				{
		...
		let mut start = 0;
		while let Some(SyntaxElement::Token(token)) = elements.get(start) {
			if !token.kind().is_trivia() {
				break;
			}
			self.trivia(&mut prefix, token);
			start += 1;
		}
		// Leading comments buffer through the pending slot; flush them
		// here so they land before the indent region, not inside it.
		self.push(&mut prefix, nil());
		...
		for (at, &element) in elements.iter().enumerate().skip(start) {
			...
			let docs = segments.last_mut().expect("segment");
			match element {
				SyntaxElement::Token(token) if token.kind().is_trivia() => {
					self.trivia(docs, token)
				}
		...
		if segments.len() == 1 {
			prefix.append(&mut segments.pop().expect("segment"));
			return concat(prefix);
		}
		let mut parts = Vec::with_capacity(segments.len() * 2 - 1);
		for (at, segment) in segments.into_iter().enumerate() {
			if at > 0 {
				parts.push(soft_line_or_space());
			}
			parts.push(concat(segment));
		}
		prefix.push(indent(fill(parts)));
		concat(prefix)
	}
...
impl ListJoiner {
	fn new() -> Self {
		ListJoiner { next_sep: None, tight_next: false, any: false }
	}
