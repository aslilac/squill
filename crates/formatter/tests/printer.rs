...
use formatter::doc::*;
use formatter::{IndentStyle, KeywordCase, MAX_WIDTH, Options, render};
...
fn fill_breaks_only_where_needed() {
    ...
    assert_eq!(
        lines.len(),
        2,
        "six 18-char words fill two lines: {rendered}"
    );
    assert!(lines.iter().all(|line| line.chars().count() <= MAX_WIDTH));
    assert!(lines[0].contains("word-0000000000000") && lines[0].contains("word-0000000000003"));
}
...
fn nested_groups_break_outside_in() {
    ...
    let inner = group(concat([text("inner"), soft_line_or_space(), text("bits")]));
