...
pub fn lex_with(
	...
	while lexer.pos < lexer.bytes.len() {
		...
		tokens.push(Token { kind, text: &input[start..lexer.pos] });
	}
	tokens
}
...
impl Lexer<'_> {
	...
	fn next_token(&mut self) -> SyntaxKind {
		...
		match b {
			...
			b'e' | b'E'
				if self.dialect == Dialect::Postgres && self.at(1) == Some(b'\'') =>
			{
				self.bump(1);
			...
			b'%'
				if self.options.pyformat_params
					&& self.pyformat_param_len().is_some() =>
			{
				let len = self.pyformat_param_len().expect("checked");
				self.bump(len);
				SyntaxKind::Param
			}
			_ if self.dialect == Dialect::Postgres && is_pg_op_byte(b) => {
				self.pg_operator()
			}
			// `_` is ASCII punctuation but starts an identifier.
			_ if self.dialect == Dialect::Sqlite
	...
	fn pyformat_param_len(&self) -> Option<usize> {
		...
		if self.at(1) == Some(b'(') {
			let mut i = 2;
			while self.at(i).is_some_and(|b| b != b')' && b != b'\n' && b != b'%') {
				i += 1;
			}
			if self.at(i) == Some(b')')
