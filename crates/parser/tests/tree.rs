...
use parser::tree::Cst;
use parser::tree::Event;
use parser::tree::build_tree;
...
fn statement_texts(cst: &Cst) -> Vec<String> {
	...
	root.statements().map(|statement| statement.syntax().to_string()).collect()
}
...
";
	...
	assert!(
		stmts[1].trim_start().starts_with("-- name: ListAPIKeys :many"),
