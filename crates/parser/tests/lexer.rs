...
use parser::Dialect::Postgres;
use parser::Dialect::Sqlite;
use parser::Dialect::{self};
...
fn operator_may_end_in_plus_minus_only_with_special_char() {
	...
	assert_tokens(
		Postgres,
		"a=-b",
		&[(K::Ident, "a"), (K::Operator, "="), (K::Operator, "-"), (K::Ident, "b")],
	);
}
...
fn comment_openers_terminate_operator_munch() {
	assert_tokens(
		Postgres,
		"1+--x",
		&[(K::Number, "1"), (K::Operator, "+"), (K::LineComment, "--x")],
	);
	assert_tokens(
		Postgres,
		"=/*c*/",
