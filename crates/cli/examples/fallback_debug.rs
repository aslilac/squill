...
fn main() {
    ...
    for node in parse.cst.root().children() {
        ...
        let ok_tokens =
            formatter::check::tokens_equivalent(&original, &rendered, options.dialect, lex_options);
        ...
        println!("=== fallback (tokens_ok={ok_tokens} comments_ok={ok_comments}) ===");
        let a: Vec<_> = parser::lexer::lex_with(&original, options.dialect, lex_options)
            .into_iter()
            .filter(|t| !t.kind.is_trivia())
            .map(|t| (t.kind, t.text.to_string()))
            .collect();
        let b: Vec<_> = parser::lexer::lex_with(&rendered, options.dialect, lex_options)
            .into_iter()
            .filter(|t| !t.kind.is_trivia())
            .map(|t| (t.kind, t.text.to_string()))
            .collect();
        ...
        if !ok_comments {
            let ca = formatter::check::comment_texts(&original, options.dialect, lex_options);
            let cb = formatter::check::comment_texts(&rendered, options.dialect, lex_options);
