//! `squill.toml` discovery and parsing (TREE-105).
//!
//! The config surface is exactly the CLI's option surface — nothing new
//! sneaks in through config. The format is a deliberately flat TOML
//! subset: `key = value` lines, `#` comments, no sections, no arrays.
//! Parse errors and unknown keys are hard errors with file:line.

use std::path::{Path, PathBuf};

use formatter::{IdentQuoting, IndentStyle, KeywordCase};
use parser::Dialect;
...
pub fn parse_config(text: &str, path: &Path) -> Result<PartialOptions, String> {
    ...
    for (index, raw) in text.lines().enumerate() {
        let lineno = index + 1;
        let err = |message: String| format!("{}:{lineno}: {message}", path.display());
        let line = raw.trim();
        ...
        match key {
            ...
            "indent" => {
                options.indent_style = Some(parse_indent(&string(&value)?).map_err(&err)?);
            }
            "indent-width" => match value {
                Value::Integer(n) if (1..=16).contains(&n) => {
                    options.indent_width = Some(n as u8);
                }
                _ => return Err(err("`indent-width` expects an integer from 1 to 16".into())),
            },
            "keyword-case" => {
                options.keyword_case = Some(parse_keyword_case(&string(&value)?).map_err(&err)?);
            }
            "quote-idents" => {
